...
L'intérêt d'une baseline se comprend surtout par comparaison. Supposons qu'un modèle ARIMA obtienne un RMSE de 1 000 ventes sur l'ensemble test. Ce chiffre est difficile à interpréter isolément. En revanche, si le modèle naïf obtient un RMSE proche de 1 000, on conclut que le modèle complexe n'améliore quasiment pas la précision malgré sa complexité. À l'inverse, si la baseline obtient un RMSE de 3 000, le même RMSE de 1 000 représente un gain majeur, ce qui justifie pleinement la modélisation avancée.
...
\begin{enumerate}
    ...
    \item de calibrer l'interprétation des métriques dans l'unité du problème, ici le nombre de ventes mensuelles.
\end{enumerate}
...
\end{lstlisting}
Pour le modèle final \texttt{SARIMA(1,1,0)x(1,1,0,12)}, les modules des racines AR sont supérieurs à 1. Le plus petit module saisonnier est environ \textbf{1.099} et le module non saisonnier est environ \textbf{2.887}. Cela confirme la stationnarité de la partie autorégressive après les différenciations appliquées.
...
$\phi_1$ & 0.6856 & 0.6834 \\
$\phi_2$ & -0.2699 & -0.2668 \\
Constante équivalente & 2 794.24 & 2 732.71 \\
RMSE train & 2 130.78 & 2 131.57 \\
\bottomrule
\end{tabular}
\caption{Comparaison des paramètres AR(2) estimés manuellement et avec statsmodels.}
\end{table}

Les coefficients $\phi_1$ et $\phi_2$ sont presque identiques entre les deux approches. La constante reste également du même ordre de grandeur. La proximité du RMSE d'entraînement confirme que l'implémentation manuelle capture correctement la logique autorégressive.
...
\midrule
$\theta_1$ & 0.7000 & 0.6881 \\
$\sigma^2$ & 4 529 696.11 & 4 528 391.00 \\
\bottomrule
\end{tabular}
\caption{Comparaison des paramètres MA(1) estimés manuellement et avec statsmodels.}
\end{table}

Les écarts entre les deux implémentations sont faibles. \texttt{statsmodels} utilise une estimation par maximum de vraisemblance conditionnel ou exact, tandis que l'implémentation from scratch utilise une procédure itérative simplifiée. Malgré ces différences, les paramètres obtenus sont très proches, ce qui valide la logique pédagogique de l'implémentation manuelle.

\subsection{Interprétation des paramètres du modèle final}
Pour le modèle \texttt{SARIMA(1,1,0)x(1,1,0,12)} retenu, les paramètres estimés sont approximativement :
\[
\phi_1=-0.346, \qquad \Phi_1=-0.322.
\]
Le coefficient $\phi_1$ négatif indique qu'après différenciation, une augmentation importante d'un mois peut être suivie d'une correction partielle le mois suivant. Cela correspond à un mécanisme de retour vers un niveau plus normal après un choc de vente. Le coefficient saisonnier $\Phi_1$ négatif indique également une correction à l'échelle annuelle : une variation forte par rapport au même mois de l'année précédente tend à être partiellement compensée.

Ces paramètres sont cohérents avec la série de champagne : les ventes sont fortement dominées par des cycles annuels, notamment les pics de fin d'année. Le modèle final capture cette dynamique saisonnière beaucoup mieux que les modèles ARIMA non saisonniers.
